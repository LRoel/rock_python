...
class NavProcess():
    def __init__(self, stop):
        ...
        locations = collections.OrderedDict()
        path = path_map.get_path((self.loc.update_loc('map').x, self.loc.update_loc('map').y, self.loc.update_loc('map').theta), stop)
        rospy.loginfo("path is " + str(path))
        if len(path) > 1:
            locations = path_map.get_pose(path)
...
def handle_nav_path(req):
    ...
    NavProcess((req.navgoal.x, req.navgoal.y, req.navgoal.z))
