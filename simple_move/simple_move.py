...
class Simple:
    ...
    def go_zero_frame_2(self, trace_frame):
        if self.loc_.update_loc(trace_frame).y < 0:
            k = (abs(self.loc_.update_loc(trace_frame).y)) ** 3
            rate = rospy.Rate(10)
            n = 0
    ...
    def go_zero_frame_3(self, trace_frame):
        rate = rospy.Rate(10)
        while self.loc_.update_loc(trace_frame).x > 0.1:
            if self.loc_.update_loc(trace_frame).y < -0.02:
                while self.loc_.update_loc(trace_frame).y < -0.02 and self.loc_.update_loc(trace_frame).x > 0.1:
                    self.cmd_vel(-0.08, -0.05 - math.fabs(self.loc_.update_loc(trace_frame).y)/2)
                    # print (self.loc_.x - old_x) ** 2 + (self.loc_.y - old_y) ** 2
                    rate.sleep()
                self.cmd_vel(0, 0)
            elif self.loc_.update_loc(trace_frame).y > 0.02:
                while self.loc_.update_loc(trace_frame).y > 0.02 and self.loc_.update_loc(trace_frame).x > 0.1:
                    self.cmd_vel(-0.08, 0.05 + math.fabs(self.loc_.update_loc(trace_frame).y)/2)
                    # print (self.loc_.x - old_x) ** 2 + (self.loc_.y - old_y) ** 2
                    rate.sleep()
